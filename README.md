# Fundamentos de Matemática para Ingeniería de Audio

**Autor:** Federico Daverio  
**Correo:** <daveriofederico@gmail.com>

---

## Descripción

La matemática constituye uno de los pilares fundamentales en la formación de un **Ingeniero de Audio**. Los conceptos desarrollados en este curso proporcionan las herramientas necesarias para comprender y modelar fenómenos presentes en la acústica, el procesamiento digital de señales (DSP), la electrónica, el análisis de sistemas y el diseño de equipos de audio.

El objetivo de este repositorio es recopilar el material del curso **Fundamentos de Matemática para Ingeniería de Audio**, ofreciendo las presentaciones utilizadas en clase como material de consulta para estudiantes y docentes.

---

## Temario

| Tema | Duración | Presentación |
|:------|:--------:|:------------:|
| Unidades Fundamentales y Derivadas | 2 h | [📄 Slides](slides/01_Unidades_Fundamentales.pdf) |
| Jerarquía de Operaciones | 1.5 h | [📄 Slides](slides/02_Jerarquia_Operaciones.pdf) |
| Fracciones | 2 h | [📄 Slides](slides/03_Fracciones.pdf) |
| Relaciones y Funciones | 3 h | [📄 Slides](slides/04_Relaciones_y_Funciones.pdf) |
| Vectores | 2.5 h | [📄 Slides](slides/05_Vectores.pdf) |
| Sistemas Lineales y Matrices | 4 h | [📄 Slides](slides/06_Sistemas_Lineales_y_Matrices.pdf) |

---

## Contenido del curso

### 1. Unidades Fundamentales y Derivadas

- Sistemas de medición
- Unidades fundamentales y derivadas
- Notación científica
- Notación de ingeniería
- Conversiones simples

📄 **Presentación:** [01_Unidades_Fundamentales.pdf](slides/01_Unidades_Fundamentales.pdf)

---

### 2. Jerarquía de Operaciones

- Símbolos de agrupación
- Potencias y raíces
- Productos y cocientes
- Sumas y restas

📄 **Presentación:** [02_Jerarquia_Operaciones.pdf](slides/02_Jerarquia_Operaciones.pdf)

---

### 3. Fracciones

- Componentes de una fracción
- Fracciones simples y compuestas
- Mínimo Común Múltiplo (MCM)
- Máximo Común Divisor (MCD)

📄 **Presentación:** [03_Fracciones.pdf](slides/03_Fracciones.pdf)

---

### 4. Relaciones y Funciones

- Relaciones entre variables
- Clasificación de funciones
- Estudio de funciones
- Representación gráfica

📄 **Presentación:** [04_Relaciones_y_Funciones.pdf](slides/04_Relaciones_y_Funciones.pdf)

---

### 5. Vectores

- Concepto de vector
- Notación vectorial
- Plano cartesiano
- Coordenadas polares
- Conversión entre sistemas

📄 **Presentación:** [05_Vectores.pdf](slides/05_Vectores.pdf)

---

### 6. Sistemas Lineales y Matrices

- Ecuaciones lineales
- Sistemas de ecuaciones
- Matrices y determinantes
- Métodos de resolución

📄 **Presentación:** [06_Sistemas_Lineales_y_Matrices.pdf](slides/06_Sistemas_Lineales_y_Matrices.pdf)

---

## Estructura del repositorio

```text
.
├── README.md
├── slides/
│   ├── 01_Unidades_Fundamentales.pdf
│   ├── 02_Jerarquia_Operaciones.pdf
│   ├── 03_Fracciones.pdf
│   ├── 04_Relaciones_y_Funciones.pdf
│   ├── 05_Vectores.pdf
│   └── 06_Sistemas_Lineales_y_Matrices.pdf
└── recursos/
```

---

## Objetivos de aprendizaje

Al finalizar este curso, el estudiante será capaz de:

- Comprender el uso de sistemas de unidades y realizar conversiones.
- Aplicar correctamente la jerarquía de operaciones.
- Resolver operaciones con fracciones.
- Analizar relaciones y funciones matemáticas.
- Representar y operar vectores en diferentes sistemas de coordenadas.
- Resolver sistemas de ecuaciones lineales utilizando distintos métodos matriciales.

---

## Licencia

Este material ha sido elaborado con fines educativos para el curso de **Fundamentos de Matemática para Ingeniería de Audio**.
